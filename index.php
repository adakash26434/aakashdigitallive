...
$__logoClients = [];
try { $__logoClients = query("SELECT org_name, logo_url FROM clients WHERE status='active' AND logo_url IS NOT NULL AND logo_url!='' ORDER BY " . sqlRand() . " LIMIT 20"); } catch (\Throwable $e) { error_log('[' . basename(__FILE__) . ']' . $e->getMessage()); }
// Fallback to the partners directory (client logos / names) when no CRM client logos exist
if (empty($__logoClients)) {
  try { $__logoClients = query("SELECT name AS org_name, logo_url FROM partners WHERE active=1 AND type='client' ORDER BY position ASC, id ASC LIMIT 24"); } catch (\Throwable $e) { error_log('[' . basename(__FILE__) . ']' . $e->getMessage()); }
}
