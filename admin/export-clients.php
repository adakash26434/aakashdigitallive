...
if ($q) {
    $where .= " AND (c.client_code LIKE ? OR c.org_name LIKE ? OR c.contact_name LIKE ? OR c.email LIKE ? OR c.district LIKE ?)";
    $p = "%$q%";
...
try {
    $rows = query(
        "SELECT
            ...
            c.expiry_month       AS 'Expiry Month',
            c.phone              AS 'Phone 1',
            c.phone2             AS 'Phone 2',
...
if (!empty($rows)) {
    ...
    fputcsv($out, [
        ...
        'Agreement Date','Installation Date','Expiry Month',
        'Phone 1','Phone 2','Mobile 1','Mobile 2',
        ...
        'CBS Use','Integration','Integration Charge','Package','Status',
    ]);
}
